...
function signUp (req,res){
  ...
  User.findOne({email: req.body.email},(err,userR)=>{
    ...
    if(userR) return res.status(404).send({message: 'El usuario ya se registro con ese email'})
    User.findOne({idCard: req.body.idCard},(err,userR)=>{
      if(err) return res.status(500).send({message:err})
      if(userR) return res.status(404).send({message: 'El usuario ya se registro con esa cedula'})
